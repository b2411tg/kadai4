...
diff /tmp/$$-ans /tmp/$$-result || ERROR_EXIT "result err" # 失敗
rm -f /tmp/$$-*
echo "正常系　引き数１と引き数２をマイナスで起動した時: OK"
...
diff /tmp/$$-ans /tmp/$$-result || ERROR_EXIT "result err" # 失敗
rm -f /tmp/$$-*
echo "異常系　引き数を一つだけで起動した時: OK"

# 異常系　引き数を無しで起動した時
./greatest_common_divisor.sh > /tmp/$$-result
echo "入力エラー！ 引き数がないか、数字以外の文字が入力されました。" > /tmp/$$-ans
diff /tmp/$$-ans /tmp/$$-result || ERROR_EXIT "result err" # 失敗
rm -f /tmp/$$-*
echo "異常系　引き数を無しで起動した時: OK"

# 異常系　引き数１を数字以外で起動した時
./greatest_common_divisor.sh test 248 > /tmp/$$-result
echo "入力エラー！ 引き数がないか、数字以外の文字が入力されました。" > /tmp/$$-ans
diff /tmp/$$-ans /tmp/$$-result || ERROR_EXIT "result err" # 失敗
rm -f /tmp/$$-*
echo "異常系　引き数１を数字以外で起動した時: OK"

# 異常系　引き数２を数字以外で起動した時
./greatest_common_divisor.sh 248 test > /tmp/$$-result
echo "入力エラー！ 引き数がないか、数字以外の文字が入力されました。" > /tmp/$$-ans
diff /tmp/$$-ans /tmp/$$-result || ERROR_EXIT "result err" # 失敗
rm -f /tmp/$$-*
echo "異常系　引き数２を数字以外で起動した時: OK"
